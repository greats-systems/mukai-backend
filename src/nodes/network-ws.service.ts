...
@Injectable()
export class CoreProduceNetworkWsService
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private subscribeToProduceManager() {
    ...
    try {
      ...
      this.logger.error(
        '❌ Failed to Subscribe produce manager contract',
        error,
      );
    }
  }
  async checkSubscriptionExists(name: string) {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/v1/subscriptions?name=${name}`,
      );
      return res.data.length > 0;
    } catch (error) {
      console.warn(`Could not connect to subscription service ${error}`);
    }
  }
