...
export class OrdersDemandService {
  ...
  async update(id: string, updateOrderDto: UpdateDemandOrderDto) {
    try {
      ...
      var offer_item = updateOrderDto.offer
      const now = new Date().toISOString();

      const { error: updateError, data: orderData } = await this.postgresRest
        .from('demands_requests')
        .update({ 'status': updateOrderDto.status, })
        .eq('id', id).select('*,demands_requests_customer_id_fkey(*), demand_request_items(*)').single();


      if (orderData) {
        console.log('updateOrderDto order', orderData['demand_request_items'][0]['name']);
        ...
        if (order.status == 'accepted') {
          ...
          console.log('updateOrderDto cancelled_by_customer orderData', orderData);
        ...
        const { data: updatedOrderData } = await this.postgresRest
          .from('demands_requests')
          .select('*, demand_request_items(*)').eq('id', id).single();
        return {
          status: 'success',
          message: 'order created successfully',
  ...
  async notifyCustomerDemand(order: UpdateDemandOrderDto, message: string) {
    try {
      ...
      try {
        var customerProfile = await this.userService.getUser(order.customer_id)
        // Send notifications
        console.log('customerProfile?.customer_id!', customerProfile?.id!);
        console.log('customerProfile?.push_token!', customerProfile?.push_token!);
  ...
  async broadcastDemandCancellation(order: UpdateDemandOrderDto, message: string) {
    try {
      // get the profiles to broadcast to by order category and location
      const { error: profilesError, data: profiles } = await this.postgresRest
        .from('profiles')
