...
export class ProfileSeeder {
    ...
    private generateMockProfiles(count: number): any[] {
        ...
        const countries = ['USA', 'Canada', 'UK', 'Germany', 'France', 'Japan', 'Australia', 'South Africa', 'Brazil', 'India'];
        const cities = {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const city = cities[country] ? cities[country][Math.floor(Math.random() * cities[country].length)] : 'Unknown';
            ...
            const profile: Profile = {
                ...
                abilities: ['Programming', 'Design', 'Marketing', 'Sales'][Math.floor(Math.random() * 4)],
                department: ['IT', 'HR', 'Finance', 'Operations'][Math.floor(Math.random() * 4)],
                role: ['Developer', 'Manager', 'Analyst', 'Director'][Math.floor(Math.random() * 4)],
                account_type: ['Personal', 'Business'][Math.floor(Math.random() * 2)],
                specialization: ['Web Development', 'Data Science', 'UX Design'][Math.floor(Math.random() * 3)],
                trading_as: i % 3 === 0 ? `Company ${i + 1}` : null,
                is_subscribed: Math.random() > 0.7,
                is_premium_subscribed: Math.random() > 0.9,
                community_role: ['Member', 'Moderator', 'Admin'][Math.floor(Math.random() * 3)],
